/*
 * macOS graphics adapter for the original gr_* API.
 * Based on Parallax's portable Macintosh/PPC code; DOS VGA register and BIOS
 * operations are deliberately absent. SDL remains isolated in sdl_shim.c.
 */
...
static unsigned char macos_video_memory[320 * 400];
unsigned char *gr_video_memory = macos_video_memory;
static descent_sdl *macos_sdl;
...
{
	if (macos_sdl != NULL)
		descent_sdl_present_6bit(macos_sdl, gr_video_memory, gr_visible_pal);
}
...
*/
	w = 320; r = 320; h = mode == SM_320x400U ? 400 : 200; t=BM_LINEAR; data = (intptr_t)gr_video_memory;
	video_set_res(w, h);
...
int gr_init(int mode)
{
	...
	macos_sdl = descent_sdl_create("Descent");
	if (macos_sdl == NULL)
		return 11;
