/*
 * Portable replacement contract
 * -----------------------------
 * Replaces: FIX/FIX.ASM
 * Exports/state: fix_fastsincos, fix_sincos, fix_asin, fix_acos, fix_atan2, long_sqrt, quad_sqrt, and fix_sqrt.
 *
 * The DOS module passes arguments through Watcom register conventions and
 * performs 16.16 fixed-point operations in 32-bit x86 registers.  This sibling
 * is Parallax's C implementation from the Macintosh/PPC source lineage.  The
 * MACOS build compiles this file and explicitly excludes the assembly module,
 * so behavior can be validated side-by-side against the unchanged original.
 */
