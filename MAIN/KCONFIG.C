...
#if defined(MACOS)
/* Keep the control model and configuration UI authoritative; use WASD and
 * enable mouse Y inversion through the normal profile fields. */
{0xc8,0x48,0xd0,0x50,0xcb,0x4b,0xcd,0x4d,0x38,0xff,0x1e,0x4f,0x20,0x51,0xff,0x4a,0xff,0x4e,0xff,0xff,0x10,0x47,0x12,0x49,0x1d,0x9d,0x39,0xff,0x21,0xff,0x11,0xff,0x1f,0xff,0x30,0xff,0x13,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xf,0xff,0xff,0xff,0xff,0xff},
#else
...
#if defined(MACOS)
{0x0,0x1,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0x1,0x1,0x0,0x0,0xff,0x0,0xff,0x0,0xff,0x0,0xff,0x0,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},
#else
{0x0,0x1,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0x1,0x0,0x0,0x0,0xff,0x0,0xff,0x0,0xff,0x0,0xff,0x0,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff},
#endif
...
void controls_read_all()
{
	...
	if (Config_control_type==5 ) {
		...
		mouse_get_delta( &dx, &dy );
#if defined(MACOS)
		/* SDL relative motion is already expressed in screen-independent
		 * mouse counts.  The DOS divisors make modern mouse input roughly
		 * four times weaker before the sensitivity setting is applied. */
		mouse_axis[0] = (dx*FrameTime)/8;
		mouse_axis[1] = (dy*FrameTime)/8;
#else
...
//----------- Clamp values between -FrameTime and FrameTime
	...
	if (Config_control_type == CONTROL_MOUSE) {
		...
		pitch_limit *= MACOS_MOUSE_ACCELERATION_MULTIPLIER;
		heading_limit *= MACOS_MOUSE_ACCELERATION_MULTIPLIER;
	}
#endif

	if (Controls.pitch_time > pitch_limit ) Controls.pitch_time = pitch_limit;
	if (Controls.vertical_thrust_time > FrameTime ) Controls.vertical_thrust_time = FrameTime;
	if (Controls.heading_time > heading_limit ) Controls.heading_time = heading_limit;
