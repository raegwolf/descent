...
{
	GameBitmaps = (grs_bitmap *)esp32_alloc_psram(sizeof(*GameBitmaps) * MAX_BITMAP_FILES);
	GameSounds = (digi_sound *)esp32_alloc_psram(sizeof(*GameSounds) * MAX_SOUND_FILES);
	SoundOffset = (int *)esp32_alloc_psram(sizeof(*SoundOffset) * MAX_SOUND_FILES);
	AllBitmaps = (BitmapFile *)esp32_alloc_psram(sizeof(*AllBitmaps) * MAX_BITMAP_FILES);
	AllSounds = (SoundFile *)esp32_alloc_psram(sizeof(*AllSounds) * MAX_SOUND_FILES);
	GameBitmapOffset = (int *)esp32_alloc_psram(sizeof(*GameBitmapOffset) * MAX_BITMAP_FILES);
	GameBitmapFlags = (ubyte *)esp32_alloc_psram(sizeof(*GameBitmapFlags) * MAX_BITMAP_FILES);
